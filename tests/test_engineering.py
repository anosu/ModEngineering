...
from mod import generated, load, solution, sync, within
spec=importlib.util.spec_from_file_location('new_mod',Path(__file__).resolve().parents[1]/'scripts/new-mod.py')
...
class EngineeringTests(unittest.TestCase):
    def test_rejects_paths_outside_repository(self):
        with tempfile.TemporaryDirectory() as directory:
            with self.assertRaises(ValueError): within(Path(directory),'../outside')

    def test_generated_configuration_detects_drift(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory)
            config=new_mod.scaffold(root,'Example-Android','Example','android','example/Example-Android')
            sync(root,config)
            sync(root,config,check=True)
            (root/'global.json').write_text('{}')
            with self.assertRaisesRegex(ValueError,'global.json'): sync(root,config,check=True)

    def test_solutions_include_test_and_fixed_runtime_projects(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory)
            config=new_mod.scaffold(root,'Example-Android-Variant','Example','android','example/Example-Android-Variant')
            config.update(useExtension=True,tests=['tests/Example.Tests/Example.Tests.csproj'])
            solution(root,config)
            projects=[p.attrib['Path'] for p in ET.parse(root/'Example-Android-Variant.slnx').getroot()]
    ...
    def test_android_template_preserves_package_contract_and_single_version(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory)
            config=new_mod.scaffold(root,'Example-Android','Example','android','example/Example-Android')
            self.assertEqual(['Mods/Example/Example.dll','Mods/Example/Utility.dll'],[p['destination'] for p in config['package']['files']])
            project=ET.parse(root/config['project']).getroot()
            self.assertEqual('1.0.0',project.findtext('PropertyGroup/Version'))
            self.assertNotIn('const string Version',(root/'src/Example/Core/ModInfo.cs').read_text())

    def test_pc_template_has_explicit_deploy_and_no_android_package(self):
        with tempfile.TemporaryDirectory() as directory:
            config=new_mod.scaffold(Path(directory),'Example','Example','pc','example/Example')

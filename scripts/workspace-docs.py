...
for name in json.loads(args.inventory.read_text(encoding='utf-8')):
    ...
    if not repo.is_relative_to(root):raise ValueError('Inventory path escaped workspace')
    config=json.loads((repo/'mod.json').read_text(encoding='utf-8'))
    project=(repo/config['project']).resolve()
    if not project.is_relative_to(repo):raise ValueError('Project path escaped repository')
    version=ET.parse(project).getroot().findtext('PropertyGroup/Version') or 'SDK 默认 1.0.0'
    libraries='—' if config['kind']=='library' else 'Utility、Extension' if config.get('useExtension') else 'Utility'
    rows.append(f'| [{name}]({name}/) | {config["kind"]} | {version} | {libraries} |')
readme=root/'README.md'
text=readme.read_text(encoding='utf-8')
start='<!-- mod-projects:start -->'
end='<!-- mod-projects:end -->'
if text.count(start)!=1 or text.count(end)!=1:raise ValueError('README must contain one project-table marker pair')

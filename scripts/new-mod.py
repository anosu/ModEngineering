...
"""
        write(
            root / f"src/{assembly}/Core/ModInfo.cs",
            f'''namespace {assembly};
...
''',
        ...
        config["package"] = {
            ...
            "files": [
                {
                    "source": f"output/{assembly}.dll",
                    "destination": f"Mods/{assembly}/{assembly}.dll",
                },
...
"""
        ...
        write(
            root / "Build.local.props.example",
            "<Project><PropertyGroup><GameDir>PATH_TO_GAME</GameDir></PropertyGroup></Project>",
        )
    ...
    write(
        root / "dependencies/README.md",
        "仅加入当前游戏和加载器必需的编译引用。Android 使用 scripts/sync-dependencies.ps1 同步，完整导出放入忽略的 interop-backup。字体及其他发行资源必须显式加入 mod.json 的 package.files。",
    )
    write(
        root / "README.md",
...
def main() -> None:
    ...
    if args.upstream:
        ...
        run(
            [
                ...
                "submodule",
                "add",
                "https://github.com/anosu/ModEngineering.git",
                "shared/ModEngineering",
            ],
            root,
        )
        run(
            ["git", "submodule", "add", "https://github.com/anosu/Utility.git", "shared/Utility"],
            root,
        )
    ...
    print(
        f"Created {root}. Add game dependencies, validate, then commit and push. No release was created."
    )

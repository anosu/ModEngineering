...
for name in json.loads(args.inventory.read_text(encoding='utf-8')):
    ...
    if not root.is_relative_to(workspace): raise ValueError('Inventory path escaped workspace')
    config=load(root)
    try:
        if args.command=='upgrade':
            ...
            result=subprocess.run(['git','status','--short','--branch'],cwd=root,check=True,capture_output=True,text=True)
            print(name+'\n'+result.stdout)
